...
- если день пропущен, `DailyScheduler` списывает штраф и создаёт запись в `wallet_transactions`;
...
- списание виртуального штрафа при пропущенном дне, если для обязательства включена виртуальная ответственность;
